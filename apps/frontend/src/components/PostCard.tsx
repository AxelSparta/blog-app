...
export function PostCard({ post }: { post: Post }) {
  ...
  const categoryGradient =
    categoryColors[post.category] || categoryColors.technology;
  const badgeColor =
    categoryBadgeColors[post.category] || categoryBadgeColors.technology;

  return (
    <article className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-slate-50 dark:bg-slate-950 shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1">
      {/* Image or Gradient Background */}
      <Link className="cursor-pointer" href={postLink}>
        <div className="relative aspect-video w-full overflow-hidden bg-muted">
          {post.image?.url ? (
            ...
            <div
              className={`h-full w-full bg-linear-to-br ${categoryGradient} opacity-80`}
            />
          )}
          {/* Gradient Overlay */}
          ...
          <div className="absolute top-3 left-3">
            <span
              className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium backdrop-blur-sm ${badgeColor}`}
            >
              {post.category}
            </span>
          </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <div className="flex items-center justify-between border-t border-border pt-3 mt-auto">
          ...
          <Link href={postLink} className="group flex items-center">
            <svg
